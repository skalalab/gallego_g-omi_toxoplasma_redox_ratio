...
path_output = path_project / "features"
...
for dict_dataset in tqdm(list_dataset_dicts[:]):
    ...
    for idx, row_data in tqdm(list(df_data.iterrows())[:]):#iterate through sets
        ...
        df_props.to_csv(path_output / f"{idx}.csv")  
...
#%% CREATE ONE CSV 
    ...
    # list_path_features_csv = list(path_output_proprs_type.glob("*.csv"))
