...
def plot_data(data, labels, s=3, title=""):
    ...
    for label in np.unique(labels):
        ...
        scatter = plt.scatter(
            x,
            y,
            c='#1690FF' if label == 'media'else '#FC5353',
            label='Control' if label =="media" else "Infected",
            s=s,
        )
    plt.title(title)
    plt.xlabel(x.name)
...
if __name__ == "__main__":
    ...
    df_umap, reducer_umap = compute_umap(scaled_data,
                                         ...
                                         n_neighbors=100,
                                         metric="euclidean")
    plot_data(df_umap, labels=df_data['treatment'], title="UMAP \nTimepoint 24hr ")

    # tsne
    # df_tsne, reducer_tsne = compute_tsne(scaled_data)
    # plot_data(df_tsne, labels=df_data['treatment'], title="t-SNE")

    # pca
    df_pca, reducer_pca = compute_pca(scaled_data)
    plot_data(df_pca, labels=df_data['treatment'], title="Principal Component Analysis")
